...
fn bfs(g: &Graph, map: &Map, start: Pos, end: Pos) -> Option<Path> {
    ...
    let doors = path
        ...
        .fold(0, |mask, c| mask | char_to_bit(c, b'A'));
    let keys = path
        .iter()
        .map(|&((x, y), _)| map[x][y])
...
fn compute_paths(g: &Graph, map: &Map) -> HashMap<usize, HashMap<usize, Path>> {
    let keys = g
        .keys()
        .filter(|&&(x, y)| {
            map[x][y].is_ascii_lowercase() || ['@', '%', '!', '€'].contains(&map[x][y])
        })
        .cloned()
        .collect_vec();
...
fn shortest_path(
    ...
    if let Some(&d) = cache.get(&(keys, curr)) {
        return d;
    }
    let d = paths[&curr]
        .iter()
