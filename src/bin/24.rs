...
fn bug_at(map: &[[char; 5]], (x, y): (usize, usize), (dx, dy): (i64, i64)) -> bool {
    ...
    if !(0..5).contains(&new_x) {
        return false;
    }
    if !(0..5).contains(&new_y) {
        return false;
    }
...
fn count_neighbours(bugs: &HashSet<(u8, u8, i32)>) -> HashMap<(u8, u8, i32), usize> {
    let mut count = bugs.iter().map(|&pos| (pos, 0)).collect::<HashMap<_, _>>();
    ...
    for &(x, y, z) in bugs {
        ...
        match (x, y) {
            (1, 2) => {
                for x in 0..5 {
                    neighbour_to!(x, 0, z + 1);
                }
            }
            (3, 2) => {
                for x in 0..5 {
                    neighbour_to!(x, 4, z + 1);
                }
            }
            (2, 1) => {
                for y in 0..5 {
                    neighbour_to!(0, y, z + 1);
                }
            }
            (2, 3) => {
                for y in 0..5 {
                    neighbour_to!(4, y, z + 1);
                }
            }
            _ => {}
        }
        if x != 0 && (x - 1 != 2 || y != 2) {
            neighbour_to!(x - 1, y, z);
        }
        if x != 4 && (x + 1 != 2 || y != 2) {
            neighbour_to!(x + 1, y, z);
        }
        if y != 0 && (x != 2 || y - 1 != 2) {
            neighbour_to!(x, y - 1, z);
        }
        if y != 4 && (x != 2 || y + 1 != 2) {
            neighbour_to!(x, y + 1, z);
        }
